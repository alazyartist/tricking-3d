...
export function Frank({ ...props }) {
	...
	const pos = group?.current?.children[0].children[0].position;
	const rot = group?.current?.children[0].children[0].rotation;
	const quat = group?.current?.children[0].children[0].quaternion;
	let posArr;

	useFrame(({ camera }) => {
		pos ? (posArr = [pos.x, pos.y, pos.z]) : (posArr = [0, 0, 0]);
