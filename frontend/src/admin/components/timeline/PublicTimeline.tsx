...
const PublicTimeline = ({ vidRef, source, clip: clipData }) => {
  ...
  const {
    ...
    odur,
    dur,
  } = useZoom(vidRef, clipData);

  const adjustedValue = (timelineOffset / bounds.width) * odur;
  let activeWidth = `${(
    ((parseFloat((clipData?.clipEnd as number)?.toString()) -
      parseFloat((clipData?.clipStart as number)?.toString())) /
      dur) *
    100
  )?.toFixed(2)}%`;
  let activeLeft = `${
    (((clipData?.clipStart as number) - adjustedValue) / dur) * 100
  }%`;
  return (
...
const CurrentTimeDiplay = ({ adjustedValue, dur, vidRef, start }) => {
  ...
  return (
    <input
      ...
      value={currentTime}
      min={Math.max(start, start + adjustedValue)}
      //@ts-ignore
