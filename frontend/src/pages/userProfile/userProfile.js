...
const UserProfile = () => {
	...
	return (
		<div className='m-4  pt-[3.4rem] font-inter text-zinc-300'>
			...
			<div className='border-zinc-900 border-t-4 rounded-lg'> {/* @TODO: Wrap this with profile info container */}
	 			<TricklistPage profileuuid={uuid} displayOnly />
			</div>

			<div className='fixed bottom-0 flex justify-center items-center w-[90vw] min-h-[10vh] max-h-[30vh] bg-zinc-700 border-zinc-900 border-t-4 rounded-lg'>
				{
					!selected &&
						<AddListButton setOpen={setOpenNewList} open={openNewList} /> 
				}{
					selected &&
						<AddComboItemToTricklist tricklist_id={selected.tricklist_id} addItemopen={addItemopen} setAddItemopen={setAddItemopen}/>
				}
			</div>
		</div>
	);
