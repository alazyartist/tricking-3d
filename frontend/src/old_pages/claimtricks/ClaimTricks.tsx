...
import ClaimedTrickGraph from "@components/d3/ClaimedTrickGraph";
...
const ClaimTricks = ({ user_id }) => {
  ...
  const graphRef = useClickOutside(() => {
    setSeeGraph(false);
  });
  useEffect(() => {
    console.log(uniqueTricks);
    console.log(clips);
  }, [uniqueTricksRaw]);
  return (
    <>
      <div className="no-scrollbar flex h-[60vh] w-full flex-col place-items-start overflow-y-scroll bg-zinc-900 bg-opacity-70 font-inter ">
        <div className="flex h-fit w-full place-content-center place-items-center justify-between px-2 text-center font-bold">
          <h1 className="p-2 text-2xl">ClaimTricks:</h1>
          <p className="w-full p-2 text-sm">
            {totalTricksClaimed}/{tricks?.length}
          </p>
          <button
            onClick={() => setSeeGraph(!seeGraph)}
            className="whitespace-nowrap rounded-md bg-zinc-800/40 p-1 text-xs"
          >
            see graph
          </button>
        </div>
      ...
      {seeGraph && (
        ...
        >
          <ClaimedTrickGraph
            close={() => setSeeGraph(false)}
            profileInfo={profileInfo}
            claimedTricks={uniqueTricks}
          />
        </div>
      )}
    </>
