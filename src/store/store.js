...
export const useStore = create(
	devtools((set, api) => ({
		aI: 0,
		addToAnimationArray: (value) => set((state) => ({ animationsArray: [...state.animationsArray, value], })),
		animationSelector: () => set((state) => ({ aI: (state.aI + 1) % state.animationsArray.length, isPlaying: true, isPaused: false, })),
		animationsArray: [],
		bounce: true,
		clipDuration: 0,
		currentAnim: "BTwist",
    ...
    isScrubbing: 0, // 0: No, 1: Start, 2: End
		...
		timescale: 1,
		trimToggle: false,
		updateAnimationArray: (value) => set(() => ({ animationsArray: [...value], })),
	}))
);
